...
import { addForm, fetchForms, uploadPhoto } from '../../JS/formSlice/FormSlice';
...
const [selectedCause, setSelectedCause] = useState("سرعة فائقة");
  ...
  const [formData, setFormData] = useState({
    ...
    degat: "",
    sens: "",
    nk: getDefaultNK(user?.district || "oudhref"),
    mtr: "0",
    nbrmort: "0",
    nbrblesse: "0",
    cause: "سرعة فائقة",
  ...
  useEffect(() => {
    if (user) {
        setFormData((prev) => ({ ...prev, createdBy: user._id }));
    }
  }, [user]);
  ...
  const generateOptionsNK = (userDistrict, userAutonum) => {
    let start, range;
    if (userAutonum === "a1") {
    switch (userDistrict) {
      case "oudhref":
        ...
        <Modal.Body>
          <Form.Group controlId="accident">
            ...
            <Row className="form-section km-row">
              ...
              >
                {generateOptionsNK(user?.district, user?.autonum)}
              </Form.Select>
              +
              <Form.Control
                type="number"
            ...
            <Row className="form-section">
              ...
              >
                {getSenseByDistrict(user?.district, user?.autonum).map((r) => (
 ...
 <Row className="form-section plate-section align-items-center">
      ...
      <Col xs={12} sm={8}>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={(e) => handleAddForm(e)} disabled={isUploading}>
