    ...
    const usersFromTargetRegionAndRole = users
      .filter(
        (user) =>
          ...
          (user.role === "securite" || user.role === "entretient" || user.role === "superviseur")
      )
        ...
        <Table striped bordered hover>
          ...
          <tbody>
            {startDate && endDate && sortedDataArray.length === 0 ? (
                ...
                <tr key={item._id}>
                  <td>
                    {/* Debug logs to inspecting image data */}
                    {console.log("Item ID:", item._id, "Images:", item.image)}
                        ...
                        item.image.map((src, idx) => (
                          ...
                          <img
                            ...
                            onError={(e) => {
                              ...
                              // Don't hide, just show alt to indicate broken link
                            }}
                          />
                          ) : (
                            <span key={idx} style={{fontSize: '10px', color: 'red'}}>Inv Img</span>
                          )
                        ))
                      ) : (
                        <span>No Image</span>
