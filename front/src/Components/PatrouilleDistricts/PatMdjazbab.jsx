...
export const PatMdjazbab = () => {
  ...
  const autorouteDistrictMap = {
      ...
      const userAutonum = "a3";
        ...
        const usersFromTargetRegionAndRole = userRedux
          .filter(
            (user) =>
              (user.district || "") === "mdjazbab" &&
              ((user.role || "") === "patrouille" || (user.role || "") === "superviseur")
          )
          .map((user) => user._id);
        ...
        return data.filter((form) => {
          ...
          // Infer autonum and district from the creator user, since patrouille records don't have these fields
      ...
      const filteredData = (data, start, end, userAutonum = null) => {
        ...
        return baseFiltered.filter((form) => {
          const formDateStr = getSessionDate(new Date(form.createdAt));
          const isAfterOrSameStart = !start || formDateStr >= start;
          const isBeforeOrSameEnd = !end || formDateStr <= end;
  ...
  return (
    <div>
        <h2>تحصيل مركبة دورية السلامة اقليم مجاز الباب</h2>
