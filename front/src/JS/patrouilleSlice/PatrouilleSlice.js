...
export const fetchPatrouilles = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get("https://autoroute-c4tf.vercel.app/pat/");
...
export const addPatData = createAsyncThunk("data/add", async (newData) => {
  try {
    const response = await axios.post("https://autoroute-c4tf.vercel.app/pat/", newData, {
      headers: {
...
export const deletePatData = createAsyncThunk('data/delete', async (id) => {
  try {
    await axios.delete(`https://autoroute-c4tf.vercel.app/pat/${id}`);
    return id;
  } catch (error) {
...
export const updatePatData = createAsyncThunk(
  ...
  async ({ id, patData }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`https://autoroute-c4tf.vercel.app/pat/${id}`, patData, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
