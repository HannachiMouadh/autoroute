...
export const fetchEntData = createAsyncThunk("data/fetchAll", async () => {
  try {
    const response = await axios.get("https://autoroute-c4tf.vercel.app/ent/");
...
export const addEntData = createAsyncThunk("data/add", async (newData) => {
  try {
    const response = await axios.post("https://autoroute-c4tf.vercel.app/ent/", newData, {
      headers: {
...
export const deleteEntData = createAsyncThunk("data/delete", async (id) => {
  try {
    await axios.delete(`https://autoroute-c4tf.vercel.app/ent/${id}`);
    return id;
  } catch (error) {
  ...
  async ({ id, entData }, { rejectWithValue }) => {
    try {
      const response = await axios.put(
        `https://autoroute-c4tf.vercel.app/ent/${id}`,
        entData,
        {
          headers: {
            "Content-Type": "application/json",
