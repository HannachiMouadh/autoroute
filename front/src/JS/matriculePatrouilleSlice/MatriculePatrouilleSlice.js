...
export const fetchMatricule = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get("https://autoroute-c4tf.vercel.app/matPat/");
...
export const addMatriculeData = createAsyncThunk("data/add", async (newData) => {
  try {
    const response = await axios.post("https://autoroute-c4tf.vercel.app/matPat/", newData, {
      headers: {
...
export const deleteMatriculeData = createAsyncThunk('data/delete', async (id) => {
  try {
    await axios.delete(`https://autoroute-c4tf.vercel.app/matPat/${id}`);
    return id;
  } catch (error) {
...
export const updateMatriculeData = createAsyncThunk(
  ...
  async ({ id, matriculeData }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`https://autoroute-c4tf.vercel.app/matPat/${id}`, matriculeData, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
