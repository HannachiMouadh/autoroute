...
export const fetchForms = createAsyncThunk('data/fetchAll', async () => {
  try {
    const response = await axios.get('https://autoroute-c4tf.vercel.app/auto/');
...
export const addForm = createAsyncThunk('data/add', async (newData) => {
  try {
    const response = await axios.post('https://autoroute-c4tf.vercel.app/auto/', newData, {
      headers: {
...
export const deleteForm = createAsyncThunk('data/delete', async (id) => {
  try {
    await axios.delete(`https://autoroute-c4tf.vercel.app/auto/${id}`);
    return id;
  } catch (error) {
...
export const updateForm = createAsyncThunk(
  ...
  async ({ id, data }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`https://autoroute-c4tf.vercel.app/auto/${id}`, data, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
...
export const uploadPhoto = createAsyncThunk(
  ...
  async (formDataUpload) => {
    const response = await axios.post("https://autoroute-c4tf.vercel.app/api/upload", formDataUpload, {
      headers: {
